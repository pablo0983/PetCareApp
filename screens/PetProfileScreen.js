...
const supportsKcal = (species) => {
  const sp = (species || "").toLowerCase();

  if (sp.includes("fish") || sp.includes("pez")) return false;
  if (sp.includes("reptile") || sp.includes("reptil") || sp.includes("tortuga")) return false;

  return true; // mamíferos y aves
};
const supportsSterilization = (species) => {
  const sp = species?.toLowerCase() || "";
  return !(sp.includes("fish") || sp.includes("pez") || sp.includes("bird") || sp.includes("ave"));
};

const supportsBCS = (species) => {
  const sp = species?.toLowerCase() || "";
  return !sp.includes("fish");
};
